...
### Блок G — Группы с типом и вложением (переосмысление A4)

> Заменяет исходный A4. Группы (цветные области) получают `kind`, ноды двигаются вместе с группой через ReactFlow `parentId`.
...
- **Что сделать:**
  - Расширить поиск в «Мониторинг»-вкладке: помимо `service.name` сопоставлять с `endpoint` и `labels` узлов, уже размещённых на карте.
  - Совпадение по endpoint/лейблу — подсвечивать узел на холсте (scroll-to + выделение), но не фильтровать строку палитры (узел уже на карте, не нужно показывать его в списке добавления).
  - Поиск должен работать live (без кнопки «найти»).
- **Зависимость:** H1 (общая структура панели).
- **Готово когда:** вводишь URL в поиск → узел с этим endpoint-ом подсвечивается на карте; вводишь имя сервиса → он находится в мониторинг-списке как сейчас.
